...
const { expand, chatCwd } = require("../config");
 ...
 *
 * chat.cwd can be pointed at the home directory, which is a useful working
 * scope for an agent but also contains credentials the model has no reason to
 * open. That
...
function safePath(cfg, p, { forWrite = false } = {}) {
  ...
  const candidates =
    path.isAbsolute(p) || p.startsWith("~")
      ? [expand(p)]
      : [path.join(chatCwd(cfg), p), path.join(cfg.paths.root, p)];
...
const HANDLERS = {
  ...
  run_command(cfg, args) {
    ...
    return shell.run(command, {
      cwd: chatCwd(cfg), timeout: 120000, maxBuffer: 8 * 1024 * 1024,
    }).then((r) =>
      clip((r.stdout + r.stderr).trim() || (r.error ? String(r.error) : "(no output)")));
  },
};
