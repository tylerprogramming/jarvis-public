 ...
 * rendered prompt. Everything is appended to data/logs/<name>.log. */
async function run(cfg, name, { onLog } = {}) {
  ...
  const notify = await deliver(cfg, agent, { startMs: now.getTime(), say, code });
  ...
  say(`=== done (exit ${code}) ===`);
  logStream.end();
  return { skipped: false, code, notify };
}
