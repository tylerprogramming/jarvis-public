...
function defaultFile(cfg) {
  ...
  try {
    found = fs.readdirSync(dir)
      ...
      .filter((f) => !/^(index|readme)\.md$/i.test(f))
      // never the shipped sample: agents WRITE confirmed rules back into these
      // files, and writing them into a tracked example means a new operator's
      // learned rules land in a file that conflicts on the next git pull
      .filter((f) => !/(^|\.)example\.md$/i.test(f))
      .sort()
      .map((f) => path.join(dir, f));
  } catch {}
  if (found.length) return found;
  // nothing of the operator's own yet - name the file they will own, so the
  // first rule creates it instead of landing somewhere they did not choose
  if (fs.existsSync(dir)) return [path.join(dir, "playbook.md")];
