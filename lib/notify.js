...
/* Where Telegram is. TELEGRAM_API_BASE exists so a test can point every
 * Telegram call at a loopback stub; nobody sets it otherwise. scripts/notify.py
 * and scripts/inbox.py build their URLs the same way, so the three cannot
 * disagree about where Telegram is. */
function telegramApi(method, token) {
  const base = (process.env.TELEGRAM_API_BASE || "https://api.telegram.org").replace(/\/$/, "");
  return `${base}/bot${token}/${method}`;
}

/* The chats that have messaged a bot, with their ids.
 *
 * The chat id is the one secret no settings page has: a bot learns a chat only
 * when someone writes to it, and Telegram forgets after 24 hours. `jarvis
 * notify telegram-id` and `jarvis connect` both need this, and a second copy
 * of the parsing is how one of them would come to miss group posts. Never
 * throws, and never lets the token into the error text.
 *
 * Returns { ok, chats: [{id, type, who}] } or { ok: false, error, status }. */
async function telegramChats(token, { timeout = 15000 } = {}) {
  if (!token) return { ok: false, error: "no TELEGRAM_BOT_TOKEN set", status: 0 };
  const hide = (s) => String(s == null ? "" : s).split(String(token)).join("<token>");
  let body, status = 0;
  try {
    const r = await fetch(telegramApi("getUpdates", token), { signal: AbortSignal.timeout(timeout) });
    status = r.status;
    body = await r.json();
    if (!body.ok) return { ok: false, status, error: hide(body.description || `HTTP ${r.status}`) };
  } catch (e) {
    return { ok: false, status, error: hide((e && e.message) || e) };
  }
  const seen = new Map();
  for (const u of body.result || []) {
    ...
    if (c && c.id != null && !seen.has(String(c.id)))
      seen.set(String(c.id), {
        id: String(c.id),
        type: c.type || "",
        who: c.title || [c.first_name, c.last_name].filter(Boolean).join(" ") || c.username || "",
      });
  }
  return { ok: true, status, chats: [...seen.values()] };
}

module.exports = { PROVIDERS, MODES, WHENS, channels, envSlots, resolve, envFor, agentSettings, command, telegramApi, telegramChats };
