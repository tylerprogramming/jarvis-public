...
/* Quote one argument for the shell run() will use. Single quotes on both
 * families; only the escape for a literal quote differs. Paths and titles
 * with spaces go through here rather than being string-concatenated. */
function quote(arg) {
  const s = String(arg);
...
module.exports = { shellFor, run, quote, which, venvPython, openUrl, isWindows, homedir };
