...
async function status(cfg) {
  ...
  return {
    ...
    input: await hasAudioInput(),
    browser: true,
  };
}

/* Every other check here asks whether transcription would work. None of them
 * asked whether there is a microphone to transcribe. A Mac Studio has no
 * built-in one, so doctor could report the whole voice-in chain ready on a
 * machine that cannot record a single sample, and the browser then fails with
 * NotFoundError. Cheap to check, so check it. */
async function hasAudioInput() {
  try {
    if (process.platform === "darwin") {
      const out = await sh("system_profiler", ["SPAudioDataType", "-json"], 6000);
      // Only devices that actually capture carry coreaudio_device_input. Do not
      // match on the word "input" anywhere in the blob: output-only devices
      // still mention input sources, so that reports a mic on a machine with
      // none, which is the exact lie this check exists to prevent.
      let found = false;
      JSON.stringify(JSON.parse(out), (k, v) => {
        if (k === "coreaudio_device_input" && Number(v) > 0) found = true;
        return v;
      });
      return found;
    }
    if (process.platform === "linux") {
      const out = await sh("arecord", ["-l"], 3000);
      return /card \d+/.test(out);
    }
  } catch {}
  return null; // unknown, which is not the same as no
}

function sh(cmd, args, ms) {
  return new Promise((resolve, reject) => {
    const { execFile } = require("child_process");
    execFile(cmd, args, { timeout: ms, maxBuffer: 4 << 20 }, (err, stdout) =>
