...
function capabilities(cfg, opts) {
  ...
  // skills, and promises to run all of them, whether or not the session can.
  // The Skill tool is in chat.allowed_tools, but a skill it names is only
  // usable if it is installed here. State the actual grant so the claim
  ...
  if (agents.length) {
    lines.push(
      ...
      "YOU CAN START AGENTS. When asked to run, kick off, rerun or refresh one,",
      "end your reply with a line of its own: `RUN: <name>` - several names",
      "comma separated, or `RUN: all` for every scheduled agent. Jarvis's server",
      "starts them in that order, one at a time, removes the line, and appends",
      "what actually started. Say they have been started, never that they ran",
      "or what they found: a run takes minutes and its report appears in the",
      "documents trail when it finishes. Do not say you cannot run agents.",
      "`jarvis agents install` writes a real schedule that survives reboot:",
      "confirm with the user before running it, and never run it unasked.",
    );
  } else {
    lines.push(
...
function build(cfg, opts) {
  ...
  const lines = [
    ...
    "If a number answers it, say the number and stop: 'Subscribers are 12,400,",
  ...
  /* What the operator told Jarvis to keep, and what was decided lately. Both
   * are read from files code wrote; the model is told in the same breath that
   * it cannot write them, because otherwise it says "noted, I'll remember"
   * and nothing is kept. */
  let mem = { text: "" };
  try { mem = memory.read(cfg); } catch {}
  if (mem.text) lines.push("OPERATOR MEMORY (durable facts they told you; treat as true):\n" + mem.text.trim());
  let recent = "";
  try { recent = memory.recap(cfg); } catch {}
  if (recent) lines.push("RECENT DECISIONS:\n" + recent);
  lines.push(
    'To remember something durable, tell the operator to say "remember that ...";',
    "you cannot write memory yourself.",
  );
  ...
  lines.push(
    ...
    // Chat delivery setup is the one thing people ask the HUD to help with
    // that has a fixed procedure and a secret in the middle of it. Point at the
    // doc so the walkthrough is the written one, and forbid the guess that
    // costs the most: a Telegram chat id is discovered, never inferred.
    `SETTING UP CHAT DELIVERY (discord, telegram, slack, ntfy): follow ${root}/docs/NOTIFY.md`,
    "step by step. Ask them to paste the webhook URL or bot token; write it to .env,",
    "never config.json. Never guess a Telegram chat id: have them message the bot,",
    "then run `jarvis notify telegram-id`. Finish with `jarvis notify test <channel>`",
    "and report what it actually printed.",
    // The inbox is the other direction and people ask for it in the same
    // breath. Without this the model knows how to send them a report and not
    // how to let them reply, which reads as the feature not existing.
    "TALKING TO JARVIS FROM A PHONE: that is the inbox, and it is separate from",
    "delivery. `scripts/inbox.py` polls Telegram from this machine and forwards",
    "only allow-listed chats to the local server, so nothing has to be exposed to",
    "the internet: no port forwarding, no tunnel, no webhook. It needs the same two",
    "secrets as Telegram delivery (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID in .env).",
    "Set it up with `jarvis inbox start`, check it with `jarvis inbox status`, and",
    "`jarvis inbox install` keeps it running across reboots. In that chat, /new",
    "starts a fresh conversation, /status reports agent health, and /run <agent>",
    "runs one. Never suggest binding the server to 0.0.0.0 or forwarding a port",
    "to achieve this; the inbox exists so that is never necessary.",
  );
  ...
  /* Headings only, never the body. The first brain file used to be inlined
   * whole when under 4 KB, which cost every turn those tokens whether or not
   * the answer needed them, and the second and third files got nothing. A
   * section index tells the model what each file covers so it can Read the
   * one section it needs. */
  for (const f of brainFiles) {
    const index = memory.headingIndex(f);
    prompt += `\n\nKNOWLEDGE FILE ${f} (read on demand${index ? `; sections:\n${index}` : ""})`;
  }
