...
const FAILURE_SIGNATURES = [
  ...
  /rate limit/i,
  /permission prompt/i,
  /* A usage cap ends the turn with an apology and exit 0, so without these
   * the run is recorded as a clean success that happens to have written
   * nothing. Kept in sync with lib/brain/limits.js, which classifies the same
   * failures for chat. */
...
/* The HUD asks for every agent's health every 8 seconds, and health() reads
 * the ledger once per agent. A stat is far cheaper than a read and a parse,
 * so the parsed rows are kept until the file's mtime moves. Callers filter
 * or copy the array; nobody appends to it in place. */
let ledgerCache = { file: null, mtime: null, rows: [] };
function read(cfg) {
  const file = ledgerFile(cfg);
  let mtime = null;
  try { mtime = fs.statSync(file).mtimeMs; } catch {}
  if (ledgerCache.file === file && ledgerCache.mtime === mtime) return ledgerCache.rows;
  const rows = readJson(file, []);
  ledgerCache = { file, mtime, rows: Array.isArray(rows) ? rows : [] };
...
function health(cfg, agent, now = new Date()) {
  ...
  /* A named failure outranks a clean exit code. A usage cap, an expired
   * login and an overloaded server all end the turn politely and exit 0, so
   * judging on the code alone reports the run as fine and the agent as ok -
   * including for a quiet_ok agent like the watchdog, whose whole job is to
   * notice this. The error text is already the runner's own words. */
  if (lastRow.error) {
    out.state = "failed";
    out.detail = `${fmtLocal(lastMs)}: ${lastRow.error}`;
    return out;
  }
