...
/* Where the chat brain starts. Empty means the repo root, so chat and the
 * scheduled agents read the same CLAUDE.md and the same project memory. Both
 * brains call this rather than expanding chat.cwd themselves. */
function chatCwd(cfg) {
  const set = expand(String((cfg.chat || {}).cwd || "").trim());
  return set || (cfg.paths && cfg.paths.root) || ROOT;
}
...
module.exports = { load, merge, readJson, expand, save, chatCwd, loadEnvFile, ROOT, HOME };
