...
(function () {
  ...
  /* A brain failure is not an answer.
   *
   * app.js renders the `error` event as "[runner error <code>]" appended to
   * whatever streamed, and a usage cap or an expired login arrives as a
   * normal-looking result, so the failure text used to sit in the transcript
   * looking like something Jarvis said. The server now classifies these and
   * sends kind/label/message/hint; show them as a card that says what broke
   * and what fixes it, and put the input back so the person can retry after.
   */
  function brainErrorCard(d) {
    const box = document.createElement("div");
    box.className = "brainerr" + (d.kind ? " k-" + d.kind : "");
    const head = document.createElement("div");
    head.className = "brainerr-head";
    head.textContent = d.label || "the brain failed";
    box.appendChild(head);
    if (d.message) {
      const msg = document.createElement("div");
      msg.className = "brainerr-msg";
      msg.textContent = d.message;
      box.appendChild(msg);
    }
    if (d.hint) {
      const hint = document.createElement("div");
      hint.className = "brainerr-hint";
  ...
  /* app.js owns the stream loop and writes errors straight into the reply
   * node. Rather than fork that whole function, watch for the node it
   * produces and replace it: the marker text is stable and only ever written
   * on the error branch. */
  const chatlog = document.getElementById("chatlog") || document.querySelector(".chatlog");
  if (chatlog && window.MutationObserver) {
    let lastErr = null;
    window.__jarvisBrainError = (d) => { lastErr = d; };
    new MutationObserver(() => {
      const nodes = chatlog.querySelectorAll(".msg .txt, .reply, .bubble");
      for (const n of nodes) {
        const t = n.textContent || "";
        const m = t.match(/\[runner error ([\s\S]*)\]\s*$/);
        if (!m || n.dataset.brainerr) continue;
        n.dataset.brainerr = "1";
        const before = t.slice(0, m.index).trim();
        n.textContent = before;
        const d = lastErr && lastErr.code === m[1] ? lastErr : { label: "the brain failed", message: m[1] };
        n.parentNode.appendChild(brainErrorCard(d));
        lastErr = null;
      }
    }).observe(chatlog, { childList: true, subtree: true, characterData: true });
  }

  /* app.js keeps only `code` from the error event, so the classified payload
   * (kind, label, hint) would be lost. Tee the response stream - the page
   * still reads its own copy - and hand the full object to the observer. */
      ...
      try {
        for (;;) {
          ...
          while ((i = buf.indexOf("\n\n")) >= 0) {
            ...
            if (!/^event: error$/m.test(chunk)) continue;
            const dm = chunk.match(/^data: (.+)$/m);
            if (dm) { try { window.__jarvisBrainError(JSON.parse(dm[1])); } catch {} }
          }
        }
      } catch { /* the page's copy is what matters; this one is best effort */ }
    })();
    return new Response(theirs, { headers: res.headers, status: res.status, statusText: res.statusText });
  };
